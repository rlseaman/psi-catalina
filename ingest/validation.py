...
CATALINA_SCHEMA=os.path.join(SCHEMA_PATH, 'PDS4_CSS_1A10.xsd')
...
CATALINA_SCHEMATRON=os.path.join(SCHEMA_PATH, 'PDS4_CSS_1A10.sch')
...
def run_validator(file_name):
    p = subprocess.run(['validate', 
        '-D', 
        '-m', MODEL_VERSION,
        '-x', IMAGING_SCHEMA, DISP_SCHEMA, GEOM_SCHEMA,
        '-S', IMAGING_SCHEMATRON, DISP_SCHEMATRON, GEOM_SCHEMATRON,
        '-s', 'json'
